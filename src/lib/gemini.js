/**
 * Google Gen AI Developer API client for resume parsing, optimization and LaTeX generation.
...
/**
 * Common fetch wrapper for Google Gemini API
 */
async function callGeminiAPI(systemPrompt, userPrompt, apiKey) {
  const key = apiKey || import.meta.env.VITE_GEMINI_API_KEY;
  if (!key) {
    throw new Error('Gemini API key is missing. Please configure VITE_GEMINI_API_KEY in your .env or enter it in the header.');
  }
  ...
  if (!parts || parts.length === 0) {
    throw new Error('Received an empty text part from Gemini API.');
  }
...
Return JSON in this EXACT structure:
{
  ...
  "atsAssessment": {
    "score": 85,
    "feedback": [
      "Naturally matched and injected 12 critical keyword parameters into resume summary.",
      "Quantified achievements successfully with metrics inside job descriptions."
    ],
    "missingKeywords": [
      "Kubernetes", "Redis"
    ]
...
Now, compile this raw data into an optimized, quantified, ATS-optimized JSON resume according to the requested schema. Ensure to output ONLY valid JSON.`;
  ...
  const rawJsonText = await callGeminiAPI(systemPrompt, userPrompt, apiKey);
  ...
  try {
    ...
    console.error('Failed to parse Gemini output as JSON. Raw output was:', rawJsonText);
...
Convert it into a beautiful, ready-to-compile moderncv LaTeX document in the 'banking' style with 'blue' theme color.`;
  ...
  const rawLatex = await callGeminiAPI(systemPrompt, userPrompt, apiKey);
