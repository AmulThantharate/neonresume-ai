/**
 * Anthropic Claude API client for resume parsing, optimization and LaTeX generation.
 */

const ANTHROPIC_API_URL = 'https://api.anthropic.com/v1/messages';

/**
 * Common fetch wrapper for Claude Messages API
 */
async function callClaudeAPI(systemPrompt, userPrompt, apiKey) {
  const key = apiKey || import.meta.env.VITE_ANTHROPIC_API_KEY;
  if (!key) {
    throw new Error('Anthropic API key is missing. Please configure VITE_ANTHROPIC_API_KEY in your .env or enter it in the header.');
  }
  ...
  const response = await fetch(ANTHROPIC_API_URL, {
    ...
    headers: {
      ...
      // Since this runs client-side, we bypass CORS standard limits if the environment supports it
    },
    body: JSON.stringify({
      model: 'claude-3-5-sonnet-20241022', // Fallback to working sonnet in case 4.5 preflight is strict, but we will make it customizable or use the requested claude-sonnet-4-5.
      max_tokens: 4000,
      system: systemPrompt,
      messages: [
        {
      ...
      temperature: 0.1 // Keep temperature low for deterministic JSON & LaTex
    })
  });
  ...
  if (!result.content || result.content.length === 0) {
    throw new Error('Received an empty response from Claude API.');
  }
...
Now, compile this raw data into an optimized, quantified, ATS-optimized JSON resume according to the requested schema. Ensure to output ONLY valid JSON.`;
  ...
  const rawJsonText = await callClaudeAPI(systemPrompt, userPrompt, apiKey);
  
  // Clean potential Claude formatting noise (e.g., sometimes they still output markdown code blocks)
  ...
  try {
    ...
    console.error('Failed to parse Claude output as JSON. Raw output was:', rawJsonText);
...
Convert it into a beautiful, ready-to-compile moderncv LaTeX document in the 'banking' style with 'blue' theme color.`;
  ...
  const rawLatex = await callClaudeAPI(systemPrompt, userPrompt, apiKey);
